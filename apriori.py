...
def loadDataSet():
    import  codecs
    fr = codecs.open ( 'words.txt', 'r', 'utf_8' )
    rows=[]
    while 1:
        line = fr.readline()
        if not line:
...
def scanD(D, Ck, minSupport):
    import copy
    ssCnt = {}
    listD=list(copy.deepcopy(D))
    listC1=list(copy.deepcopy(Ck))
    for tid in listD:
        for can in listC1:
            if can.issubset(tid):
                #print(str(can)+'\t'+str(tid))
                if can not in ssCnt:
                    ssCnt[can]=1
                 #   print(can)
                else:
                    ssCnt[can] =int(ssCnt[can])+ 1
            else:
                pass
    numItems = float(len(list(copy.deepcopy(D))))
    print('numItems:'+str(numItems))
    retList = []
    supportData = {}
    print('ssCnt:'+str(ssCnt))
    for key in ssCnt:
